...
protocol.derive({ ...base("b"), provenance: [], validity: { status: "FRESH", checkedAt: at, policy: "MANUAL" }, dependsOn: ["a"] });
protocol.registerValidator({ id: "test", validate: async (source) => ({ memoryId: "a", result: source.version.token === "v1" ? "UNCHANGED" : "CHANGED", checkedAt: at, version: source.version }) });
...
const report = await protocol.validate(["a"], { a: { memoryId: "a", result: "UNCHANGED", checkedAt: at, version: { scheme: "test", token: "v2" } } });
