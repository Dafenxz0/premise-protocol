...
export class GitValidator {
  ...
  versionFor(sourceUri: string): GitVersion {
    ...
    return { scheme: source.objectPath.endsWith("/") ? "git.tree" : "git.blob", token: object };
  }
  ...
  async validate(source: SourceReference & { readonly memoryId?: string }): Promise<ValidationResult> {
    ...
    try {
      ...
      if (message.includes("bad object") || message.includes("ambiguous argument") || message.includes("does not exist") || message.includes("not a git repository")) return { memoryId, result: "MISSING", checkedAt: new Date().toISOString(), sourceUri: source.sourceUri };
  ...
  private git(repositoryPath: string, args: readonly string[]): string {
    return execFileSync("git", ["-C", repositoryPath, ...args], { cwd: repositoryPath, encoding: "utf8" }).trim();
  }
}
