...
credential-like environment variables and passes that scrubbed environment as
the complete child environment; it does not merge `process.env` again. It sets
network and credential controls to disabled, and never supplies the repository
checkout, benchmark answers, or an oracle. A sentinel-secret regression test
fails if a scrubbed value reaches the child. The evaluator rejects private
imports, repository-relative imports, credential literals, undeclared input
reads, and candidates that do not use the public SDK package.
...
The credential isolation regression can be run with:

```powershell
pnpm adoption:codex-luna:security-test
```
...
committed. CI runs the sealed contract and sentinel regression only; it does
not launch a live Codex/Luna command, so ordinary CI remains deterministic and
credential-free while a separately authorized Codex/Luna run can be audited
