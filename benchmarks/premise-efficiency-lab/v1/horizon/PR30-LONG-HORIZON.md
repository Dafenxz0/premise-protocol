...
Status: **measurement pass; evidence-integrity correction required before stronger claims**.
...
recording decisions. It measures active records separately from the retained
event and decision structures, frontier tombstones, receipt state and
...
The independent oracle runs in a separate Node process without importing the
runner or runtime. It receives the raw runtime snapshot and decision trace,
replays event status/dependency transitions, checks event structure and
version continuity, and returns only a boolean verdict to the public report.
The tampering tests mutate an event version or dependency and must fail. The
generated report stays under `.tmp/`; its manifest records the result digest,
oracle-evidence digests, commit, Node version, platform and configuration.
...
in-memory runtime, not merely an external historical archive. They grow with
the horizon while the active record count and frontier structures stay bounded
in this fixture. The snapshot-clone column shows the extra heap required to
hand raw evidence to the isolated oracle; neither column is a production
capacity guarantee or a peak-memory measurement.
...
The public result contains observed counters, raw-state digests and the oracle
verdict, but not the oracle's expected counts or internal labels. A failed
oracle process produces `INCONCLUSIVE` and a non-zero CLI exit status.

## Interpretation rule

`eventCount` and `decisionEvents` are retained live memory in this runtime.
Because they grow linearly with the horizon while active records and frontier
state remain bounded, PR31 is **required for evaluation**. It is not permission
to claim that compaction exists, that the negative cache is bounded, or that
